...
public class NotificationService {
   ...
   // studentRepository.findByClazz_IdAndIsDeleted(classActivity.getClazz().getId(),
   // false);
   // for (Student student : students) {
   // if (ObjectUtils.isNotEmpty(student.getEmail())) {
   // try {
   // emailService.sendHtmlEmail(student.getEmail(), "Notification",
   // EmailTemplateConstant
   // .generateNotificationTemplate(student.getName(),
   // classActivity.getActivityTime().toString()));
   ...
   public void setIsReadAll() {
      ...
      List<NotificationRecipient> notificationRecipients = notificationRecipientRepository
            .findByAccountAndIsRead(account, false);
